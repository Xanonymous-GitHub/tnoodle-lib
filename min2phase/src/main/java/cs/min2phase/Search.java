...
public class Search {
    ...
    protected int initPhase2Pre() {
        ...
        for (
            int p2switch = 0, p2switchMask = (1 << p2switchMax) - 1;
    ...
    protected int initPhase2(int p2corn, int p2csym, int p2edge, int p2esym, int p2mid, int edgei, int corni) {
        int prun = Math.max(
            CoordCube.getPruning(
                ...
                CoordCube.CCombPConj[CubieCube.Perm2CombP[corni >> 4] & 0xff][CubieCube.SymMultInv[edgei & 0xf][
                    corni &
                    0xf]]
            ),
            Math.max(
                CoordCube.getPruning(
                    ...
                    CoordCube.CCombPConj[CubieCube.Perm2CombP[p2corn] &
