    ...
    void doSearch() {
        ...
        do {
            ...
            for (length12 = p1SolsArr[0].value; length12 < 100; length12++) {
                for (FullCube fullCube : p1SolsArr) {
                    if (fullCube.value > length12) {
                        break;
                    }
                    if (length12 - fullCube.length1 > MAX_LENGTH2) {
                        continue;
                    }
                    c1.copy(fullCube);
                    ...
                    int s2ct = ct2.getct();
                    int s2rl = ct2.getrl();
                    length1 = fullCube.length1;
                    length2 = length12 - fullCube.length1;
